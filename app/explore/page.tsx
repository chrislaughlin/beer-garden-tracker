...
import { BelfastMapPanel } from '@/components/maps/belfast-map-panel';

const activeFilters = ['Evening sun', 'Food', 'Dog friendly'];
  ...
  return (
    <div className="space-y-5">
      <div className="sticky top-4 z-10 rounded-[2rem] bg-white/90 p-4 shadow-soft backdrop-blur">
        ...
        <div className="mt-3 flex flex-wrap gap-2 text-sm">{['Map', 'List', ...activeFilters].map((item) => <span key={item} className="rounded-full bg-accent px-4 py-2 font-medium text-amber-900">{item}</span>)}</div>
      </div>
      <BelfastMapPanel title="Use the map when you’re actually out and about" description="Map/list toggle, geolocation, and quick-scan cards are designed to stay legible outdoors on a phone." />
