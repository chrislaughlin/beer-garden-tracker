...
export default function HomePage() {
  ...
  const summary = beerGardenService.getHomepageSummary();
  return (
    <div className="space-y-6">
      ...
      <section className="grid gap-4 sm:grid-cols-3">
        {[
          ['Visible spots', String(summary.totalVisible)],
          ['Evening sun picks', String(summary.sunnyNow)],
          ['Awaiting approval', String(summary.awaitingApproval)]
        ].map(([label, value]) => <div key={label} className="rounded-[2rem] bg-white/80 p-5 shadow-soft"><p className="text-sm text-slate-500">{label}</p><div className="mt-2 text-3xl font-bold">{value}</div></div>)}
      </section>
